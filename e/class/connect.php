...
function page1($num,$line,$page_line,$start,$page,$search){
	...
	$newsearch = substr($search,1);
	parse_str($newsearch);
	
	if($mid)
	{
		return page11($num,$line,$page_line,$start,$page,$search);
	}
	$search = '.html';
	if($searchid)
		$search = 's-'.$searchid.'.html';
		
	if($classid)
	{
		$search = 'c-'.$classid.'.html';
	}
	if($userid)
	{
		$search = 'u-'.$userid.'.html';
	}
		
	//print_r($fun_r['trecord']);die;
	$url=$public_r['newsurl'].'page';
	$snum=2;//最小页数
...
//前台分页
function page11($num,$line,$page_line,$start,$page,$search){
	global $fun_r;
	if($num<=$line)
	{
		return '';
...
function sys_ReturnBqTitleLink($r){
	...
	if(empty($r['isurl']))
	{
			if($class_r[$r[classid]][showdt]==2)//add by liangpingzheng 2013-08-08
			{
				if($class_r[$r[classid]][tbname]=="news")
				{
					$char = array('#',",", ".", "/", ";", "'", "[", "]", "\\", "=", "`", "<", ">", "?", ":", "\"", "{", "}", "|", "+", ")", "(", "*", "&", "^", "%", "$", "!", "~", "，", "。", "、", "；", "‘", "【", "】", "=", "）", "（", "*", "&", "……", "%", "￥", "！", "~", "·", "《", "》", "？", "：", "“", "{", "}");
					$title = str_replace($char,'',$r[title]);
					$title = str_replace(' ','-',$title);
	
					$titleurl=$public_r[newsurl]."product-{$title}-$r[classid]-$r[id].html";
				}
				else
				{
					$titleurl=$r['titleurl'];
